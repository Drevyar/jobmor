...
select pg_temp.assert_true((select count(*)=1 from public.jobs where title='Employer test job'),'owner creates and reads job');
update public.jobs set title='Edited employer test job' where title='Employer test job';
select pg_temp.assert_true((select count(*)=1 from public.jobs where title='Edited employer test job'),'owner edits job');
...
select pg_temp.assert_true((select count(*)=1 from public.applications where applicant_id=auth.uid()),'student applies');
do $$ begin
  begin
...
select pg_temp.assert_true((select count(*)=0 from public.jobs where title='Edited employer test job'),'other employer cannot read');
select pg_temp.assert_true((select count(*)=0 from public.applications where applicant_id='10000000-0000-4000-8000-000000000003'),'other employer cannot read applications');
select pg_temp.assert_true((select count(*)=0 from public.profiles where id='10000000-0000-4000-8000-000000000003'),'other employer cannot read applicant contact');
do $$ begin
...
select set_config('request.jwt.claims','{"sub":"10000000-0000-4000-8000-000000000001","role":"authenticated"}',true);
select pg_temp.assert_true((select count(*)=1 from public.profiles where id='10000000-0000-4000-8000-000000000003'),'owner reads applicant contact');
select pg_temp.assert_true((select count(*)=0 from public.profiles where id='10000000-0000-4000-8000-000000000004'),'owner cannot read unrelated student');
...
select pg_temp.assert_true((select status='accepted' from public.applications where applicant_id='10000000-0000-4000-8000-000000000003'),'accept works');
...
select pg_temp.assert_true((select status='rejected' from public.applications where applicant_id='10000000-0000-4000-8000-000000000003'),'reject works');
do $$ begin
...
select public.update_employer_profile('New contact','0812345678','Updated company','events','New address');
select pg_temp.assert_true((select display_name='New contact' from public.profiles where id=auth.uid()),'contact saved');
select pg_temp.assert_true((select company_name='Updated company' from public.employer_profiles where id=auth.uid()),'company saved');
do $$ begin
  begin
...
select pg_temp.assert_true((select display_name='New contact' from public.profiles where id=auth.uid()),'invalid profile leaves no partial change');
delete from public.jobs where title='Edited employer test job';
select pg_temp.assert_true((select count(*)=0 from public.applications where applicant_id='10000000-0000-4000-8000-000000000003'),'job delete cascades applications');
set local role anon;
do $$ begin
...
reset role;
rollback;
select 'Employer CRUD, applications, profile and ownership checks passed; fixtures rolled back.' as result;
